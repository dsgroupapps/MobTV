...
const PRIMARY_NAVIGATION: NavigationItem[] = [
  ...
  { label: "Campanhas", to: "/dashboard/campanhas", icon: Megaphone },
  { label: "Pedidos", to: "/dashboard/pedidos", icon: ShoppingBag },
  { label: "Mídias", to: "/dashboard/midias", icon: Film },
  { label: "Calendário", to: "/dashboard/calendario", icon: CalendarDays },
];
...
function DashboardNavigation({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex flex-1 flex-col" aria-label="Navegação da área do anunciante">
  ...
  return (
    <div className="flex h-full flex-col bg-navy text-white">
      <div className="flex h-20 items-center border-b border-white/10 px-7">
        <Link to="/dashboard" onClick={onNavigate} aria-label="Ir para a visão geral">
          ...
          <div className="flex h-20 items-center justify-between gap-4 px-5 sm:px-8 lg:px-10">
            <div className="flex min-w-0 items-center gap-3">
              <Sheet open={isMobileOpen} onOpenChange={setIsMobileOpen}>
                ...
                >
                  <SheetTitle className="sr-only">Navegação da área do anunciante</SheetTitle>
